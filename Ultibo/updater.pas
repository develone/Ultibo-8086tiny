...
begin
  ...
  // only update if pin 18 is connected to ground
  GPIOPullSelect(GPIO_PIN_18,GPIO_PULL_UP);
  GPIOFunctionSelect(GPIO_PIN_18,GPIO_FUNCTION_IN);
  if (GPIOInputGet(GPIO_PIN_18) = GPIO_LEVEL_HIGH) then
    exit;
  ...
  try
    if not data.HTTPClient.GetStream('http://10.0.1.111:8080/kernel7.img', data.FileStream) then
    begin
